...
为了避免细微更改都进行整体重新布局，浏览器将布局分为全量布局与增量布局。
...
通常情况下，当前节点布局，会同时触发其关联节点（祖先、后代节点以及兄弟节点）布局。
多数布局本质都会造成页面重新渲染。
...
* Javascript
  JS用于变更页面元素，包括显示调整，动画加入等，这一部分也可由CSS动画，渐变以及部分Web Animation API来实现

* Style
  这部分为样式计算，计算样式与哪些节点相对应。
